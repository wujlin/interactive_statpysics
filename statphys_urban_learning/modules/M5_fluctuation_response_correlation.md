...
M5 给了你一个通用测量工具：**涨落的大小 = 响应的强度**（更一般是协方差）。这句话本身不要求“有相互作用”：即使自由度彼此独立，它也成立——只是你得到的响应通常是温和的、可控的。

M6 将提出一个更尖锐的问题：**当粒子之间存在相互作用时，这个工具会告诉你什么？**  
答案是：它会第一次把“集体模式”的出现暴露出来——协方差不再是独立项的简单叠加，而会被空间关联放大。靠近相变点时，这种放大可以变得极端：你会看到易感性 \(\chi\) 变大，涨落不再是局部的，而呈现跨尺度的协同涨落（相关长度 \(\xi\) 增长；其极限行为在 M7 中会用有限尺寸标度系统讲清楚）。
