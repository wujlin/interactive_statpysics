...
这里的观念转折来自**全同粒子（indistinguishability）**：对全同粒子系统，“粒子 A 在能级 1、粒子 B 在能级 2”与“A 在 2、B 在 1”不是两个不同的物理状态；可观测量不依赖粒子标签。于是，“谁在哪里”这类描述天然包含冗余信息；**唯一不冗余的微观态描述**是：每个单粒子能级（或模式）\(r\) 上有多少粒子 —— 记为占据数 \(n_r\)。
