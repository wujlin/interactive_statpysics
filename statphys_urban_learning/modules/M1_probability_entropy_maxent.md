...
## 推荐学习顺序
1. 先读 Introduction 与 Part 1–4（主讲义主线，约 15–25 分钟）。
2. 再读阅读指南：[[Swendsen_Ch5_Probability]]（把连续概率/δ/变量变换/Bayes 这套工具箱补齐）。
3. 再读阅读指南：[[Jaynes_1957_MaxEnt]]（把 MaxEnt 的“推断视角”补齐）。
4. 最后做习题：`exercises/written/E01_maxent_to_boltzmann.md` 与 `exercises/notebooks/E01_maxent_discrete_numeric.ipynb`。
...
## 阅读材料（带读，建议）
本模块正文只讲 **主线教学（Part 1–4）**：从“计数压倒一切”到“最大熵 = 最不武断的推断”，再到“拉格朗日乘子如何把约束翻译成指数族”。

如果你希望把工具与史观补齐（强烈建议），按顺序读两份带读即可：
- [[Swendsen_Ch5_Probability]]：连续概率/δ/变量变换/Bayes（保证你在后续系综里不会在“概率密度/约束”上踩坑）
- [[Jaynes_1957_MaxEnt]]：MaxEnt 的推断视角（解释为什么指数族不是技巧，而是“缺信息时最诚实的选择”）
...
微观力学方程（牛顿方程或薛定谔方程）在时间上是完全可逆的。如果把电影倒放，原路返回的粒子依然遵守物理定律。  
**但是**，宏观世界却充满了不可逆：墨水滴入水中会扩散，但绝不会自发聚拢；城市早高峰的拥堵一旦形成，也很难自发“倒放”回去。

这个“微观可逆 vs 宏观不可逆”的矛盾（Loschmidt Paradox），曾让 Boltzmann 备受攻击。统计物理的起点不是再发明一条“趋向平衡的力”，而是换一种问法：  
**当我们缺信息时，哪些宏观规律是“计数上压倒性更可能”的结果？又该如何把“已知约束”翻译成“合理的概率分布”？**
...
### Interactive：熵与计数（可选）
不要只相信直觉，请亲自试一试。  
下方有两个盒子（左 L / 右 R）。初始时所有 50 个球都在左边（高度有序）。点击 **Start Mixing**，系统会随机挑选一个球移动到另一边。请观察：
1. 球会自发地变得左右均匀吗？
2. 一旦均匀（约 25/25）后，它还能自发回到“全在左边”的状态吗？

<InteractiveConcept type="entropy-counter" />

### 关键观察
你刚才看到的不可逆过程，并没有依靠任何“趋向平衡的力”。甚至可以说，根本没有“热力学第二定律”这条硬性规则，有的只是**概率的碾压**。
...
现在问题来了：如果系统不仅受粒子数守恒约束，还受**能量守恒**（或平均能量）约束；当你不知道每个粒子的具体位置时，你应该猜它在什么状态？

1957 年，Jaynes 提出：如果你选了一个熵（不确定性）不是最大的分布，这意味着你在没有证据的情况下，隐式地假设了系统“偏好”其内部的某些结构。  
...
Part 2 告诉我们“选最大熵分布”，但还没说**怎么求**。当约束不止一条（归一化、平均能量、已知边际……）时，我们需要一个系统的方法——这就是拉格朗日乘子法。
...
这里的乘子 $\lambda_k$ 正好对应热力学里的强度量（如 $\beta = 1/kT$）。这一步把“数学优化”与“物理可观测”接在了一起。

通用模板与示例推导分别见：[[最大熵推出指数族分布（通用模板）]]、[[Boltzmann 分布的最大熵推导]]。

> 这也是 MaxEnt 与正则系综的合流点：同一份信息，两种出发语言（见：[[MaxEnt 与正则系综等价（信息等价）]]；M3 会给出正则系综的物理推导）。
...
用 MaxEnt 推导，你会发现最合理的猜测是
$$
T_{ij} = A_i O_i B_j D_j
$$
（重力模型的原型）。这不仅是模型，这是“在只给边际账本时，你能说到哪里”的信息边界：除非你有更多数据（例如成本约束），否则你不能假设比这更多的结构。
...
## 自检问题
1. 为什么“点概率为 0”但“区间概率可以非 0”？（提示：概率是对集合/区间的积分）
2. 如果只知道“均值”约束，最大熵分布是什么形式？如果再加上“方差”约束呢？
3. 配分函数 $Z$ 与拉格朗日乘子 $\lambda$ 的关系是什么？（提示：$\partial_{\lambda}\ln Z$ 会吐出什么）
4. OD 矩阵推断里，$A_i$ 与 $B_j$ 分别对应哪一组约束？
...
### 必读
- [ ] **Reading Guide**: [[Swendsen_Ch5_Probability]]（Swendsen Ch5：5.1–5.6 带读）
- [ ] **Reading Guide**: [[Jaynes_1957_MaxEnt]]（读前可先看带读；然后建议再读原文的 Abstract + Introduction）

### 习题
- [ ] **Written**: `exercises/written/E01_maxent_to_boltzmann.md`
  - 手推一遍：从 $S$ 和 $\langle E \rangle$ 约束导出 $p \propto e^{-\beta E}$。
