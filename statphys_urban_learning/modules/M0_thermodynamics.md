...
热力学最像一套“无需知道分子在干嘛也能做预测”的语言：你只管宏观可控的变量（\(T,P,N\)…），就能判断哪个过程自发、能做多少可用功。但这里的核心矛盾是：同一系统在不同实验装置下，“自然变量”不同——有时你控制的是 \(S,V\)，有时是 \(T,P\)；如果你一直抱着 \(U(S,V,N)\) 不放，很多平衡判据会写得别扭甚至写错。Gibbs 的解决办法是把“换控制变量”系统化：用 Legendre 变换构造势函数 \(F,G\)，让你在给定 reservoir 的条件下直接用极小原则和偏导读出可观测。M0 的训练目标不是背公式，而是建立条件反射：先问“我控制什么”，再选势、再写微分关系。边界也要记住：热力学只给平衡与可逆路径的账本，不告诉你涨落有多大、也不告诉你系统如何走到平衡——这些将在后续系综与随机动力学里补齐。
