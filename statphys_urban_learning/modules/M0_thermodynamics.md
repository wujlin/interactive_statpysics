...
1. **做功**（改变体积）：外界压缩系统，注入能量 \(-P dV\)（负号的直觉：压缩时 \(dV<0\)，因此 \(-P dV>0\)，内能增加）；
2. **传热**（改变熵）：外界加热系统，注入能量 \(T dS\)（可逆过程下 \(\delta Q_{\mathrm{rev}} = T dS\)，来自 [[第一定律与第二定律 First and second law|热力学第二定律]]）；
...
- 统计物理的第一步不是“解微分方程”，而是先明确**你能控制什么**（控制变量）与**你想预测什么**（响应量）。
- 热力学提供的是一套“控制 → 响应”的记账语言，但它本身不告诉你微观概率从何而来。
- 本章只要求你掌握势函数的切换规则（Legendre 变换），不要求你现在就能写出微观分布。

## 逻辑桥接（M0 → M1）
- M0 解决了“宏观变量如何记账”的问题，但还没解决“为什么这些宏观量会稳定出现”。
- M1 将用计数与最大熵解释：宏观规律来自**微观态数量的压倒性优势**。
...
### 一个最小例子：从 \(U(S)\) 到 \(F(T)\)
先用一个只含一个变量的 toy model 看清“换控制变量”到底在干什么。设
\[
U(S)=aS^2 \quad (a>0).
\]
那么
...
\]
这就是最小演示：在 \(U(S)\) 里 \(S\) 是自变量；换到 \(F(T)\) 后，\(T\) 成为自变量，而 \(S\) 变成 \(-\partial_T F\) 读出的响应量。
...
2. 为什么 \(dF = -S\,dT - P\,dV + \mu\,dN\) 里，\(S\) 的系数是负号？
3. 在城市建模中，如果你控制的是“平均人口/事件数”而不是“固定人口/事件数”，约束通常应该写成什么形式？
...
- [ ] 能解释为什么 Legendre 变换后 \(S\) 从自变量变成响应量（从 \(\partial_S U = T\) 到 \(\partial_T F = -S\)）。
- [ ] 理解“热力学势的选择取决于控制变量”这句话（先问“我控制什么”，再选势函数）。
