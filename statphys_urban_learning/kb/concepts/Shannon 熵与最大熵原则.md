...
3.  **为什么导数是 $\ln p_i$？**  
    这非常关键。因为 $H=-\sum_i p_i\ln p_i$，所以 $\frac{\partial H}{\partial p_i}=-(\ln p_i+1)$。当 $p_i\to0^+$ 时，这个边际熵增趋于 $+\infty$：给“几乎没被分配到概率”的状态补一点点，会显著增加熵；但给已经很大的 $p_i$ 再加一点点，熵增就很有限。（这里讨论离散态；连续变量的“点概率为 0”是测度问题，别混在一起。）

    因此，在不被“硬约束”（某些状态被禁止）逼到边界的前提下，熵会推动分布尽量**摊平/均匀化**。与此同时，“惩罚高能态”的平均能量/平均成本约束会通过拉格朗日乘子把概率从高代价状态挪走。两者在 $\partial_{p_i}\mathcal{L}=0$ 的平衡条件下给出 $\ln p_i$ 对约束函数的线性关系，于是最优分布自然呈现指数形式。
