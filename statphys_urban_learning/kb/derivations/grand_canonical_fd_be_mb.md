...
### 为什么占据数 \(\{n_r\}\) 是合法的？（全同粒子）
如果粒子可区分，你必须追踪“谁在能级 1、谁在能级 2”：交换粒子标签会产生新的微观态。  
但对 [[全同粒子与不可分辨性 Indistinguishability|全同粒子]]（量子气体的基本设定），交换粒子标签不改变物理状态；可观测量不依赖“粒子 ID”。因此，“谁在哪里”的描述天然包含冗余信息；**唯一不冗余的微观态描述**是：每个单粒子能级（或模式）\(r\) 上有多少粒子。

这就是占据数表示：用 \(n_r\) 表示能级 \(r\) 上的粒子数。

### 为什么要这么做？（读者收益：把变维数问题变成固定“盒子”）
...
把“允许集合”单独写成 \(\mathcal{A}\) 的好处是：你能清楚看到三大统计分布并不需要三套推导流程——推导骨架是同一条（占据数表示 \(\to\) 指数分解 \(\to\) 乘积求和），真正分岔点只有一个：**每个能级这只“盒子”允许装 \(n_r\) 的哪些取值**。
...
到第 6 步，我们已经得到经典极限下的占据数
\[
\langle n_r\rangle \approx e^{-\beta(\varepsilon_r-\mu)}.
\]
但在经典物理里能级非常密集，我们更关心**粒子在连续相空间 \((\mathbf x, \mathbf p)\)** 的分布。下面把“能级语言”翻译回“空间语言”。
...
\]
因此动量可以先积分掉，留下的就是空间密度的形状因子 \(e^{-\beta U(\mathbf x)}\)。
...
> 城市映射小提醒：当你在城市模型里看到 \(e^{-\beta \text{Cost}}\) 的形式，通常就是“动量被积掉”的结果——只剩下空间/状态代价的权重。
