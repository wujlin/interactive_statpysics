# Swendsen (2012) — An Introduction to Statistical Mechanics and Thermodynamics
...
- Eq 5.12：独立性 ⇒ `P(x|y)=P_x(x)`（教授建议的典型“锚点”）

### M3 正则系综与 ln Z
- Eq 19.17：`P(E) = (1/Z)\,\Omega(E)\,e^{-\beta E}`
...
- Eq 20.9：`P(E,N) = (1/\\mathcal{Z})\\,\\Omega\\,e^{-\\beta E + \\beta\\mu N}`
...
- Eq 20.18：`\\ln\\mathcal{Z} = \\beta P V`（可用于从 `\\mathcal{Z}` 读出压强）
