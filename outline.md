...
**过关标准**：你能用热力学关系把 (U,S,F,G) 之间互相导出来。
...
**完成标准**：你能用“写出 (Z) 或直接采样”来求平均值，并能说明关键步骤。
...
### M10 非平衡稳态与熵产生（目标：从稳态分布走向路径不可逆性）

**核心问题：**分布已经不变，为什么概率仍能沿环路持续旋转？

**你要产出：**

* 用三态环区分稳态、细致平衡与概率净流
* 计算 Markov EPR，并用局部细致平衡判断它何时具有物理熵产生含义
* 在城市映射中把 EPR 限定为时间不可逆性 proxy，不直接等同于能耗

---

### M11 FDT 违背与 Harada–Sasa（目标：从外部测量探针可见耗散）

**核心问题：**涨落比平衡响应预言得更多时，这个谱差如何变成耗散功率？

**你要产出：**

* 区分热噪声与不受同一 FDT 约束的活性驱动
* 独立计算位置关联谱与响应函数，并构造 FDT 违背谱
* 用 $\gamma\omega^2$ 权重完成 Harada–Sasa 积分
* 区分探针可见耗散、环境熵流与完整系统总熵产生
